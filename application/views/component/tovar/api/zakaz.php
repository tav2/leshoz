		...
		<div class="product-order">
			<div class="select-wrapper">
					<select name="country_id" class="country_id" onChange="if(value!=''){location=value}else{options[selectedIndex=0];}">
						<option <?php echo $activ_selector_sort_1 ?> value="welcome/magazin_project_zakaz/<?php echo $id_sub_category ?>/1">По поступлению</option>
						<option <?php echo $activ_selector_sort_2 ?> value="welcome/magazin_project_zakaz/<?php echo $id_sub_category ?>/2">По цене от дорогих</option>
						<option <?php echo $activ_selector_sort_3 ?> value="welcome/magazin_project_zakaz/<?php echo $id_sub_category ?>/3">По цене от недорогих</option>
						<option <?php echo $activ_selector_sort_4 ?> value="welcome/magazin_project_zakaz/<?php echo $id_sub_category ?>/4">По отзывам</option>
					</select>
			</div>
			<div class="select-wrapper">
				<select name="category_select" id=""  class="country_id" onChange="if(value!=''){location=value}else{options[selectedIndex=0];}">
					<option  >Выберете тип проекта</option>
					<?php foreach($sub_category_menu as $id): ?>
						<option value="<?php echo 'welcome/magazin_project_zakaz/'.$id['tovar_sub_category_id'] ?>/<?php echo $selector_sort ?>/"><?php echo $id['tovar_sub_category_name'] ?></option>
					<?php endforeach ?>
				</select>
			</div>
		</div>
